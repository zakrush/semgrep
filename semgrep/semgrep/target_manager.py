...
from semgrep.config_resolver import resolve_target
...
class Target:
    ...
    path: Path = field(converter=resolve_target)
    ...
    def files(self) -> FrozenSet[Path]:
        ...
        if self.git_tracked_only:
            try:
                ...
                return frozenset(self.path.glob("**/*"))
        else:
            return frozenset(self.path.glob("**/*"))
...
class TargetManager:
    ...
    def get_files_for_language(self, lang: Language) -> FilteredFiles:
        ...
        explicit_files = frozenset(
            target.path for target in self.targets if target.path.is_file()
        )
        lang_filtered_explicit_files = self.filter_by_language(
            lang, candidates=explicit_files
        )
        targets = files.kept | lang_filtered_explicit_files.kept

        if not self.skip_unknown_extensions:
            targets &= lang_filtered_explicit_files.removed

        filtered_files = (
            self.file_ignore.filter_paths(candidates=targets)
            if self.file_ignore
            else FilteredFiles(targets)
        )
        self.ignore_log.semgrepignored.update(filtered_files.removed)

        return FilteredFiles(filtered_files.kept, all_files - filtered_files.kept)
